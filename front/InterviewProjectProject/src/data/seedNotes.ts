...
/**
 * 初始笔记数据（首次访问时写入本地，之后以本地为准）
 * 内容取自本项目实际开发过程中的技术记录
 */
export const seedNotes: Note[] = [
  {
    id: 'note-router-lazy',
    title: 'Vue Router 路由懒加载的原理与配置',
    summary: '为什么要写 component: () => import(...)，它到底优化了什么？',
    category: 'frontend',
    tags: ['Vue', 'Vue Router', '性能优化'],
    pinned: true,
    views: 128,
...
判断标准很简单：**首屏可见 → 直接引入；需要点击跳转才能看到 → 懒加载。`
  ...
  {
    id: 'note-playwright',
    title: 'Playwright 端到端测试入门笔记',
    summary: '微软出的 E2E 框架，比 Selenium 好用在哪？核心特性速记。',
    category: 'testing',
    tags: ['Playwright', '自动化测试', 'E2E'],
    pinned: true,
    views: 96,
...
> 项目里已经装了 Cypress（create-vue 脚手架自带），后续如果要对比，可以从"自动等待"和"并行速度"两个维度实测一下。`
  ...
  {
    id: 'note-vue-naming',
    title: 'Vue 组件命名规范与 ESLint 踩坑',
    summary: '为什么组件叫 Resume 会报错？vue/multi-word-component-names 规则解析。',
    category: 'frontend',
    tags: ['Vue', 'ESLint', '规范'],
    pinned: false,
    views: 74,
...
> 规范一旦定下来就要全局一致，不然后期重构成本很高。`
  ...
  {
    id: 'note-jmeter',
    title: 'JMeter 并发压测实战记录',
    summary: '100 并发下的响应时间分析与连接池问题定位。',
    category: 'testing',
    tags: ['JMeter', '性能测试', '压测'],
    pinned: false,
    views: 63,
...
> 结论：压测不是跑完看个数字，重点是**定位瓶颈**。响应时间长的时候，先看是数据库、缓存还是网络。`
  ...
  {
    id: 'note-vite-structure',
    title: 'Vite + Vue3 项目目录结构规范',
    summary: '一套能撑住项目长大的目录组织方式，附职责划分说明。',
    category: 'tools',
    tags: ['Vite', '工程化', '项目结构'],
    pinned: false,
    views: 88,
...
> 前提是：**页面永远不直接 import data，而是通过 service 层拿数据。**`
  ...
  },
  {
    id: 'note-mysql-slow',
    title: 'MySQL 慢查询排查思路',
    summary: '从开启慢日志到 EXPLAIN 分析，一条完整的排查链路。',
    category: 'backend',
    tags: ['MySQL', '性能优化', '数据库'],
    pinned: false,
    views: 55,
...
> 排查顺序建议：慢日志定位 → EXPLAIN 分析 → 补索引 → 改写 SQL → 复查执行计划。`
  ...
  {
    id: 'note-http-https',
    title: '面试高频：HTTP 与 HTTPS 的区别',
    summary: '不只是"更安全"，要能讲清楚 TLS 握手过程。',
    category: 'interview',
